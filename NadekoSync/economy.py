...
'''
Rewrite of economyv2 for use with Nadeko '''
nadekoPath = 'C:\\Program Files\\NadekoBot\\system\\data\\NadekoBot.db'
conn = sqlite3.connect('C:\\Program Files\\NadekoBot\\system\\data\\NadekoBot.db')
c = conn.cursor()
...
class Bank:
    ...
    def withdraw_credits(self, user, amount):
        ...
        uid = user.id
        uid = int(uid)
        info = c.execute('SELECT CurrencyAmount FROM DiscordUser Where UserId = {}'.format(uid)).fetchone()
        info = str(info)
        infolength = len(info)
        cut = infolength - 2
        info = str(info[1:cut])
        ...
        if currency >= 0:
            c.execute('UPDATE DiscordUser SET CurrencyAmount = {} Where UserId = {}'.format(currency, uid))
            conn.commit()
        else:
            raise InsufficientBalance()
        conn.close()
